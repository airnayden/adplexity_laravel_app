...
String.prototype.ltrim = function (arg) {
    return this.replace(/^,+/, "");
}
...
function toSwagger(apidocJson, projectJson) {
    addExtractData(swagger, projectJson['apidoc-swagger']);
    swagger.info = addInfo(projectJson);
    swagger.host = projectJson.sampleUrl;
...
function addExtractData(swaggerObj, extraDataObj) {
    extraDataObj = extraDataObj || {};
    for (var key in extraDataObj) {
        swaggerObj[key] = extraDataObj[key];
    }
}
...
function extractPaths(apidocJson) {
    ...
    for (var i = 0; i < apiPaths.length; i++) {
        ...
        for (var j = 1; j < matches.length; j++) {
            ...
            pathKeys.push(verbs[0].description);
        }
...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
    ...
    if (verbs.parameter != undefined) {
        for (var k = 0; k < verbs.parameter.fields.Parameter.length; k++) {
            if (verbs.parameter.fields.Parameter[k].type == 'integer') {
                val = 1;
            } else if (verbs.parameter.fields.Parameter[k].type == 'boolean') {
                val = false;
            } else if (verbs.parameter.fields.Parameter[k].type == 'string') {
                val = verbs.parameter.fields.Parameter[k].field;
            }
            desc[verbs.parameter.fields.Parameter[k].field] = val;
        }
    }

    desc = JSON.stringify(desc, undefined, "\t");
